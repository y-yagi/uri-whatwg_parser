require "uri/generic"

module URI
  class WhatwgParser
    module Generic
      def initialize(scheme,
                     userinfo, host, port, registry,
                     path, opaque,
                     query,
                     fragment,
                     parser = DEFAULT_PARSER,
                     arg_check = false)
        @scheme = nil
        @user = nil
        @password = nil
        @host = nil
        @port = nil
        @path = nil
        @query = nil
        @opaque = nil
        @fragment = nil
        @parser = parser == DEFAULT_PARSER ? nil : parser

        self.set_scheme(scheme)
        self.set_host(host)
        self.set_port(port)
        self.set_userinfo(userinfo)
        self.set_path(path)
        self.query = query
        self.set_opaque(opaque)
        self.fragment=(fragment)

        self.set_path("") if !@path && !@opaque
        DEFAULT_PARSER.parse(to_s) if arg_check

        if registry
          raise InvalidURIError,
            "the scheme #{@scheme} does not accept registry part: #{registry} (or bad hostname?)"
        end

        @scheme&.freeze
        self.set_port(self.default_port) if self.default_port && !@port
      end


      def merge(oth)
        URI::DEFAULT_PARSER.join(self.to_s, oth.to_s)
      end
      alias + merge

      def scheme=(v)
        return if v.nil? || v.empty?

        parse_result = URI::DEFAULT_PARSER.parse("#{v}:", url: self, state_override: :scheme_start_state)
        set_scheme(parse_result.scheme)
      end

      def user=(v)
        return v unless v

        if host.nil? || host.empty? || scheme == "file"
          raise InvalidURIError, "cannot set user when host is nil or file schme"
        end
        set_user(URI::DEFAULT_PARSER.encode_userinfo(v))
      end

      def password=(v)
        return v unless v

        if host.nil? || host.empty? || scheme == "file"
          raise InvalidURIError, "cannot set password when host is nil or file schme"
        end
        set_password(URI::DEFAULT_PARSER.encode_userinfo(v))
      end

      def host=(v)
        return if v.nil?

        if @opaque
          raise InvalidURIError, "cannot set host with registry or opaque"
        end

        parse_result = URI::DEFAULT_PARSER.parse(v.to_s, url: self, state_override: :host_state)
        set_host(parse_result.host)
        set_port(parse_result.port)
      end

      def port=(v)
        return if v.nil?

        if v.to_s.empty?
          set_port(nil)
          return
        end

        if host.nil? || host.empty? || scheme == "file"
          raise InvalidURIError, "cannot set port when host is nil or file schme"
        end

        parse_result = URI::DEFAULT_PARSER.parse(v.to_s, url: self, state_override: :port_state)
        set_port(parse_result.port)
      end

      def path=(v)
        return if v.nil?

        if @opaque
          raise InvalidURIError, "path conflicts with opaque"
        end

        parse_result = URI::DEFAULT_PARSER.parse(v.to_s, url: self, state_override: :path_start_state)
        set_path(parse_result.path)
      end

      def userinfo=(userinfo)
        user, password = split_userinfo(userinfo)
        user = user
        password = password
      end

      def check_opaque(v)
        return v unless v

        if @host || @port || @user || @path
          raise InvalidURIError, "cannot set opaque with host, port, userinfo or path"
        end

        self.set_opaque(v)
        DEFAULT_PARSER.parse(to_s)
        true
      end
    end
  end
end

URI::Generic.prepend(URI::WhatwgParser::Generic)
